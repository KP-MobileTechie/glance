...
describe('migrateState', () => {
  it('returns a full default state from garbage input', () => {
    ...
    expect(migrateState(42).widgets).toHaveLength(4);
  ...
  it('converts legacy pos to span, ordered by y then x', () => {
    ...
    // missing kinds are added back
    expect(s.widgets.map((w) => w.kind).sort()).toEqual(['bookmarks', 'clock', 'focus', 'weatherQuote']);
